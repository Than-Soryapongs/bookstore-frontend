...
function getShippingConfig() {
  ...
  const locationRates = rawLocationRates
    ...
    .map((entry) => {
      ...
      return { matchText: normalizeText(matchText ?? ''), fee: parseMoney(feeText, defaultFee) }
    })
    .filter((entry) => entry.matchText.length > 0)

  return { defaultFee, freeThreshold, locationRates }
}
...
function getItemTitle(item: CartItem) { return item.book?.title ?? item.title ?? 'Untitled book' }
function getItemQuantity(item: CartItem) { return item.quantity ?? 1 }
...
type WorldLocation = { label: string; address: string; description: string; lat: number; lng: number }

const WORLD_BOUNDS = { west: -180, east: 180, north: 85, south: -60 }
...
function geolocationToMapPoint(point: GeoPoint) {
  ...
  return { x: clamp(x, 12, 248), y: clamp(y, 12, 228) }
}
...
async function reverseGeocodeAddress(point: GeoPoint) {
  ...
  if (!response.ok) throw new Error('Unable to resolve the selected address.')
  const payload = await response.json() as {
    display_name?: string
    address?: { house_number?: string; road?: string; neighbourhood?: string; suburb?: string; town?: string; city?: string; province?: string; state?: string; country?: string }
  }
  const addressParts = [payload.address?.house_number, payload.address?.road, payload.address?.neighbourhood, payload.address?.suburb, payload.address?.town, payload.address?.city, payload.address?.province ?? payload.address?.state, payload.address?.country].filter((part): part is string => Boolean(part && part.trim().length > 0))
  return payload.display_name ?? addressParts.join(', ')
}

// ── GoogleMapPicker ───────────────────────────────────────────────────────────

function GoogleMapPicker({ selectedPoint, livePoint, shippingAddress, isDetectingLocation, locationMessage, onPickPoint, onUseCurrentLocation }: { selectedPoint: GeoPoint | null; livePoint: GeoPoint | null; shippingAddress: string; isDetectingLocation: boolean; locationMessage: string | null; onPickPoint: (point: GeoPoint) => void; onUseCurrentLocation: () => void }) {
    ...
    <Card className="rounded-[1.8rem] border-zinc-200/80 bg-white p-5 shadow-[0_18px_42px_rgba(15,23,42,0.08)] dark:border-zinc-800 dark:bg-zinc-900">
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="outline" size="sm" className="rounded-full border-zinc-300 dark:border-zinc-700" onClick={onUseCurrentLocation} disabled={isDetectingLocation}>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_0.72fr]">
          <div className="relative overflow-hidden rounded-[1.5rem] border border-zinc-200 bg-zinc-100 shadow-inner dark:border-zinc-800 dark:bg-zinc-950">
            <iframe title="Google Maps shipping map" src={mapUrl} className="h-72 w-full rounded-[1.5rem] opacity-95" loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
            <div className="absolute inset-0 cursor-crosshair" onPointerDown={handleMapPointerDown} role="presentation" aria-hidden="true" />
            {liveMarker ? (
              <div className="pointer-events-none absolute z-10" style={{ left: `${(liveMarker.x / 260) * 100}%`, top: `${(liveMarker.y / 240) * 100}%`, transform: 'translate(-50%, -100%)' }}>
            ...
            {selectedMarker ? (
              <div className="pointer-events-none absolute z-20" style={{ left: `${(selectedMarker.x / 260) * 100}%`, top: `${(selectedMarker.y / 240) * 100}%`, transform: 'translate(-50%, -100%)' }}>
                <div className="relative">
          ...
          <div className="space-y-3 rounded-[1.5rem] border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950/50">
            ...
            {selectedLocation ? (
              <>
                ...
                <div className="rounded-2xl border border-zinc-200 bg-white p-3 text-sm text-zinc-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300">{shippingAddress || selectedLocation.address}</div>
              </>
            ) : (
            ...
            {locationMessage ? <div className="rounded-2xl border border-zinc-200 bg-white p-3 text-sm text-zinc-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300">{locationMessage}</div> : null}
            {livePoint ? <div className="rounded-2xl border border-zinc-200 bg-white p-3 text-sm text-zinc-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300">Live location captured: {livePoint.lat.toFixed(5)}, {livePoint.lng.toFixed(5)}</div> : null}
          </div>
        </div>
      </div>
    </Card>
...
export function CheckoutPage() {
  ...
  async function handleUseCurrentLocation() {
    if (!navigator.geolocation) { setLocationMessage('Geolocation is not supported in this browser.'); return }
  ...
  const summary = useMemo(() => {
    ...
    return { itemCount, subtotal, shippingFee, totalAmount: subtotal + shippingFee }
  ...
  function handleKhqrGoToCart() {
    setShowKhqrModal(false)
    setOrder(null)
    navigate('/cart', { replace: true })
  }

  function handleKhqrViewOrders() {
    setShowKhqrModal(false)
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(250,204,21,0.08),_transparent_28%),linear-gradient(180deg,#fffdf8_0%,#ffffff_35%,#f7f5ef_100%)] text-zinc-900 dark:bg-[radial-gradient(circle_at_top,_rgba(250,204,21,0.08),_transparent_28%),linear-gradient(180deg,#09090b_0%,#111827_40%,#020617_100%)] dark:text-zinc-100">
        <header className="sticky top-0 z-30 border-b border-zinc-200/70 bg-white/85 backdrop-blur-xl dark:border-zinc-800 dark:bg-zinc-950/75">
          <div className="mx-auto flex h-16 max-w-screen-xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
            <Link to="/cart" className="flex items-center gap-3"><ArrowLeft className="size-4" />Back to cart</Link>
          </div>
        </header>
        <div className="mx-auto max-w-screen-xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12"><CheckoutSkeleton /></div>
      </main>
    )
  }

  // Order confirmed (non-KHQR) success screen
        ...
        <div className="mx-auto grid max-w-screen-xl gap-6 px-4 py-10 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:py-12">
          <Card className="rounded-[1.8rem] border-zinc-200/80 bg-white p-6 shadow-[0_18px_42px_rgba(15,23,42,0.08)] dark:border-zinc-800 dark:bg-zinc-900">
            <div className="space-y-4">
              ...
              <Badge variant="secondary" className="w-fit border border-zinc-200/80 bg-white/90 text-[10px] uppercase tracking-[0.22em] text-zinc-600 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300">Order confirmed</Badge>
              <h1 className="text-3xl font-bold tracking-tight">Your order is placed</h1>
              <p className="max-w-xl text-sm leading-6 text-zinc-500 dark:text-zinc-400">Order {order.orderNumber} was submitted successfully.</p>
              ...
              <Button asChild className="rounded-full bg-zinc-900 text-white hover:bg-zinc-700 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200"><Link to="/">Continue shopping</Link></Button>
              <Button asChild variant="outline" className="rounded-full border-zinc-300 dark:border-zinc-700"><Link to="/orders">View my orders</Link></Button>
            </div>
          </Card>
          <Card className="rounded-[1.8rem] border-zinc-200/80 bg-white p-6 shadow-[0_18px_42px_rgba(15,23,42,0.08)] dark:border-zinc-800 dark:bg-zinc-900">
            <div className="space-y-5">
              ...
              <div className="space-y-3 text-sm text-zinc-600 dark:text-zinc-300">
                ...
                <div className="flex items-center justify-between"><span>Shipping</span><span>{formatPrice(order.shippingFee)}</span></div>
                <div className="flex items-center justify-between border-t border-zinc-200 pt-3 text-base font-semibold text-zinc-900 dark:border-zinc-800 dark:text-zinc-100"><span>Total</span><span>{formatPrice(order.totalAmount)}</span></div>
              </div>
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(250,204,21,0.08),_transparent_28%),linear-gradient(180deg,#fffdf8_0%,#ffffff_35%,#f7f5ef_100%)] text-zinc-900 dark:bg-[radial-gradient(circle_at_top,_rgba(250,204,21,0.08),_transparent_28%),linear-gradient(180deg,#09090b_0%,#111827_40%,#020617_100%)] dark:text-zinc-100">
      ...
      {showKhqrModal && order && (
        <KhqrPaymentModal
          ...
          onGoToCart={handleKhqrGoToCart}
          onViewOrders={handleKhqrViewOrders}
        />
      )}
      ...
      <header className="sticky top-0 z-30 border-b border-zinc-200/70 bg-white/85 backdrop-blur-xl dark:border-zinc-800 dark:bg-zinc-950/75">
        <div className="mx-auto flex h-16 max-w-screen-xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <Button asChild variant="ghost" className="rounded-full px-3">
            <Link to="/cart"><ArrowLeft className="mr-2 size-4" />Back to cart</Link>
          </Button>
          <Link to="/" className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-4">
            <Button asChild variant="ghost" size="sm" className="relative size-9 rounded-full">
              <Link to="/cart" aria-label="Open cart"><ShoppingBag className="size-4" /></Link>
            </Button>
            <CustomerAccountMenu onLogout={() => void handleLogout()} />
          </div>
        </div>
      ...
      <div className="mx-auto max-w-screen-xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
        <div className="mb-8 space-y-3">
          <Badge variant="secondary" className="w-fit border border-zinc-200/80 bg-white/90 text-[10px] uppercase tracking-[0.22em] text-zinc-600 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300">Secure checkout</Badge>
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Complete your order</h1>
          <p className="max-w-2xl text-sm leading-6 text-zinc-500 dark:text-zinc-400">Fill in shipping details and place your order against the live cart.</p>
        </div>

        {errorMessage ? (
          <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 p-6 text-sm text-red-600 dark:border-red-900 dark:bg-red-950 dark:text-red-400">{errorMessage}</div>
        ) : null}
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
              ...
              <form className="space-y-6" onSubmit={(event) => void handleSubmitCheckout(event)}>
                ...
                {paymentMethod === 'KHQR' && (
                  <div className="flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 dark:border-red-900/50 dark:bg-red-950/30">
                    <div className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-red-600">
                      <span className="text-[10px] font-bold text-white">K</span>
                    </div>
                    <div className="text-sm text-red-700 dark:text-red-300">
                      <p className="font-semibold">Pay with KHQR</p>
                      <p className="mt-0.5 text-red-600/80 dark:text-red-400/80">
                        A QR code will appear after you place the order. Scan it with any Bakong-compatible banking app within 3 minutes.
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-3">
                  <Button
                    type="submit"
                    className="rounded-full bg-zinc-900 text-white hover:bg-zinc-700 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200"
          ...
          <Card className="h-fit rounded-[1.8rem] border-zinc-200/80 bg-white p-6 shadow-[0_18px_42px_rgba(15,23,42,0.08)] dark:border-zinc-800 dark:bg-zinc-900">
            <div className="space-y-5">
              ...
              <div className="space-y-3 text-sm text-zinc-600 dark:text-zinc-300">
                <div className="flex items-center justify-between"><span>Items</span><span>{summary.itemCount}</span></div>
                <div className="flex items-center justify-between"><span>Shipping address</span><span className="max-w-[12rem] truncate text-right">{shippingAddress || 'Select a map pin'}</span></div>
                <div className="flex items-center justify-between"><span>Subtotal</span><span>{formatPrice(summary.subtotal)}</span></div>
                <div className="flex items-center justify-between"><span>Shipping</span><span>{formatPrice(summary.shippingFee)}</span></div>
                <div className="flex items-center justify-between border-t border-zinc-200 pt-3 text-base font-semibold text-zinc-900 dark:border-zinc-800 dark:text-zinc-100"><span>Total</span><span>{formatPrice(summary.totalAmount)}</span></div>
              </div>
              <div className="space-y-3">
                {cartItems.map((item) => (
